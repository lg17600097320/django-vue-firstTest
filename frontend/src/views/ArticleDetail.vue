<template>
  ...
  <div v-if="article !== null" class="grid-container">
    <div>
      ...
      <p id="subtitle">
        本文由 {{ article.auther.username }} 发布于
        {{ formatted_time(article.created) }}

        <span v-if="ifSuperuser">
            <router-link
      ...
      <div v-html="article.content_html" class="article-body"></div>
    </div>
    <div>
      <h3>目录</h3>
...
export default {
  ...
  computed: {
      ifSuperuser() {
        return localStorage.getItem('is_superuser.myblog') == 'true'
      }
  }
};
</script>
